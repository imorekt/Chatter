...
app.delete('/api/users/:username', (req, res) => {
  ...
  db.get('SELECT email FROM users WHERE username = ?', [username], (err, row) => {
    ...
    db.serialize(() => {
      db.run('DELETE FROM moments WHERE username = ?', [username]);
      db.run('DELETE FROM comments WHERE username = ?', [username]);
      db.run('DELETE FROM likes WHERE username = ?', [username]);
      db.run('DELETE FROM contacts WHERE sender_username = ? OR receiver_username = ?', [username, username]);
      db.run('DELETE FROM notifications WHERE recipient = ? OR sender = ?', [username, username]);
  ...
  db.all(query, [username, username, username, username, username, username, username], (err, rows) => {
    ...
    const chats = rows.map(r => {
      ...
      return {
        ...
        isSystem: false,
        avatar: r.avatar,
        isDeleted: r.partner_exists === null,
...
app.get('/api/favorites/:username', (req, res) => {
  ...
  db.all(query, [username, username, username], (err, rows) => {
    ...
    const partners = {};
    rows.forEach(r => {
      partners[r.partner] = { username: r.partner, avatar: r.avatar, displayName: r.display_name };
    });
